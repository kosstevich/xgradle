...
dependencies {
    ...
    implementation(rootProject.project("xgradle-core"))
...
tasks.named("pluginUnderTestMetadata") {
    dependsOn(":xgradle-core:shadowJar")
}
...
gradlePlugin{
    ...
    plugins{
        create(project.name) {
            ...
            implementationClass = "${project.group}.plugin.XGradleSbomPlugin"
        }
    }
}
...
xgradlePublishingConventions {
    ...
    projectDescription.set("${rootProject.name} plugin for generating SBOM files using xgradle")
    ...
    developer("faux", "Arseniy Kostevich", "faux@altlinux.org")
}
